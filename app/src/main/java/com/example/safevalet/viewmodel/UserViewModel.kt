...
//    private val uid = HelperUtils.getUID(application.applicationContext)\
    ...
    private val outOkTripLiveData = MutableLiveData<NetworkResults<CustomerStatusThreeModel>>()
...
fun outOkRequest(){
    viewModelScope.launch{
        outOkTripLiveData.value = repo.outOkTrip(language,user_id,"1")
    }
}
...
//                profileImage
    ...
    fun carBackStatus(){
        viewModelScope.launch{
            CallBackMyCarLiveData.value = repo.carBackStatus(user_id, language)
        }
    }


    fun callBackCarModel(){
        viewModelScope.launch{
            CallBackCarLiveData.value = repo.callBackCarModel(user_id, language)
        }
    }
    ...
    fun customerStatusOneModel(){
        viewModelScope.launch{
            CustomerStatusOneLiveData.value = repo.customerStatusOne(language, user_id)
        }
    }
    ...
    fun customerStatusThreeModel(){
        viewModelScope.launch{
            CustomerStatusThreeLiveData.value = repo.customerStatusThree(language, user_id,"1")
        }
    }


    fun customerStatusChoseMapOrScan(){
        viewModelScope.launch{
            CustomerStatusLiveData.value = repo.customerStatus(language, user_id,"1")
        }
    }
...
//    }
    ...
    fun retreveOutOk() = outOkTripLiveData
